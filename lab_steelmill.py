...
# The orders are allocated to the slabs and the load of slab is compouted
mdl.add(mdl.pack(load, slab, list(o.weight for o in ORDERS)))
   ...
   for c in allcolors:
       ...
       for i, o in enumerate(ORDERS):
           if o.color == c:
               eachColor |= (slab[i] == s)
       eachSlab += eachColor
   mdl.add(eachSlab <= MAX_COLOR_PER_SLAB)
                

# Minimize the total loss
mdl.add(mdl.minimize(sum([mdl.element(load[s], loss) for s in Slabs])))
...
optimalLoad = []
for s in Slabs:
    optimalSlab.append(msol[slab[s]])
    optimalLoad.append(msol[load[s]])
print('slab decision variable for orders')
print(optimalSlab)
print('load decision variable for orders')
print(optimalLoad)
